...
const dueDateNotifier = require('./dueDateNotifier');
...
async function startServer() {
    try {
        await connectDB({ retries: 3, delayMs: 2000 });
        ...
        // Start notifier after server and DB are ready
        try {
            dueDateNotifier.startScheduler();
            console.log('[DueDateNotifier] Started after DB connection');
        } catch (e) {
            console.error('[DueDateNotifier] Failed to start scheduler:', e && e.message ? e.message : e);
        }
    } catch (err) {
